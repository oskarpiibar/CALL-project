...
def classify_native_language(new_text_profile, error_types_relative_df, confidence_threshold=0.95):
...
def classify_texts(df, error_types_relative_df, confidence_threshold=0.95):
    ...
    for index, row in df.iterrows():
        # The row should contain the relative error frequencies, just like error_types_relative_df
        new_text_profile = row.drop(['incorrect', 'correct']).to_dict()
...
df_to_classify = pd.read_pickle('error_types_relative_frequencies_big_dataset.pkl')  # Replace with your actual file
